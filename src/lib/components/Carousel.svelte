...
<section class="relative w-full" aria-label={ariaLabel}>
  ...
  <button
    ...
    onclick={() => go(-1)}
    disabled={active === 0}
    class="hidden md:inline-flex items-center justify-center absolute top-1/2 left-2 -translate-y-1/2 w-11 h-11 rounded-fc-pill border border-fc-border bg-fc-surface text-fc-fg text-2xl cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
  >‹</button>
  <button
    type="button"
    aria-label="Next"
    ...
    class="hidden md:inline-flex items-center justify-center absolute top-1/2 right-2 -translate-y-1/2 w-11 h-11 rounded-fc-pill border border-fc-border bg-fc-surface text-fc-fg text-2xl cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
  >›</button>
  ...
  <div class="flex justify-center gap-1.5 py-3" role="tablist">
    {#each slides as _, i}
      <span
        aria-hidden="true"
        class="w-1.5 h-1.5 rounded-full {i === active ? 'bg-fc-accent' : 'bg-fc-border'}"
      ></span>
    {/each}
  </div>
</section>
